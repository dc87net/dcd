...
export i;
for i in $activeInterfaces; do
  iface="$i";
  hardware_port="$(eval $(networksetup -listallhardwareports | eval grep -B1 \'Device: $i\' | awk -F ": " '/Hardware Port/{print $2}'))"; #e.g., `en0` for Wi-Fi
...
log "${BRED}WHY ARE WE HERE?!${NC}"; exit -99;
